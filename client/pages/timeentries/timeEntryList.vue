  ...
  <list-placeholder @create="create" :showing="hasData" :loading="isLoading">
    ...
    >
      <template slot-scope="props">
        ...
        <b-table-column field="Record" label="Record">
          {{ props.row.Record }}
        </b-table-column>
...
export default {
  ...
  methods: {
    ...
    load (sort, direction, limit, pageNum) {
      this.isLoading = true
      this.$service.paged('timeEntry', sort, direction, limit, pageNum).then((data) => {
        this.pagedData = data
    ...
    create () {
      this.$router.push({ name: 'timeEntries-ID-timeEntryEdit', params: { ID: 0 } })
    },

    edit (record) {
      this.$router.push({ name: 'timeEntries-ID-timeEntryEdit', params: { ID: record.TimeEntryID } })
    }
  }
}
