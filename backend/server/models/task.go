...
func (task *Task) SaveMessages(siteULID string) error {
	return MessageHelper().SaveMany(siteULID, task.Messages)
}

func (task *Task) LoadMessages(siteULID string) error {
	return task.LoadMessagesWhere(siteULID, "task_ulid = $1 $SITEULID", task.TaskULID)
}

func (task *Task) LoadMessagesWhere(siteULID string, sql string, args ...interface{}) error {
	children, err := MessageHelper().Where(siteULID, sql, args...)
	if err != nil {
		return err
	}
	task.Messages = children
	return nil
}
